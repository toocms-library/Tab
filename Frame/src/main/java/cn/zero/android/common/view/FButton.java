...
/**
 * 扁平化按钮<br/>
 * 可以设置阴影效果（有阴影效果时按下为无阴影效果）<br/>
 * 可以设置无阴影效果（无阴影效果时按下为变暗效果）
 *
 * @author Zero @date 2014�?9�?3�?
 * @version 1.0
 */
@SuppressLint("NewApi")
    ...
    public void refresh() {
        ...
        if (isShadowEnabled) {
            if (isEnabled()) {
                ...
                pressedDrawable = createDrawable(mCornerRadius, Color.TRANSPARENT, Color.parseColor("#999999"));
                unpressedDrawable = createDrawable(mCornerRadius, Color.TRANSPARENT, Color.parseColor("#999999"));
            }
        } else {
            mShadowHeight = 0;
            if (isEnabled()) {
                ...
                pressedDrawable = createDrawable(mCornerRadius, Color.parseColor("#999999"), Color.TRANSPARENT);
                unpressedDrawable = createDrawable(mCornerRadius, Color.parseColor("#999999"), Color.TRANSPARENT);
            }
        }
        updateBackground(unpressedDrawable);
        // Set padding
